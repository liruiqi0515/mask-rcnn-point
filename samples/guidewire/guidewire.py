...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python guidewire.py train --dataset=/home/li/GuideWire/guidewire_2020 --weights=coco
...
class GuidewireDataset(utils.Dataset):
    ...
    def load_guidewire(self, dataset_dir, subset, txt_dir=TXT_DIR):
        ...
        for a in image_list:
            ...
            with open(txt_path, 'r')as f:
                ...
                box = []
                for s in string:
                    box_item = [int(i) for i in s.split(' ')[0:4]]
                    box.append(box_item)
                #################################print
    ...
    def load_mask(self, image_id, mask_dir=MASK_DIR):
       ...
       Returns:
        ...
        mask = np.zeros([info["height"], info["width"], len(info["box"])],
                        dtype=np.uint8)
        mask_now = cv2.imread(os.path.join(mask_dir, image_info["id"][:-4] + '.png'), 0)
        for i, p in enumerate(info["box"]):
